...
class Dataset_Ship_Classification(Dataset):
    def __init__(self, root_path, flag='train', size=None,
        ...
        if self.geo_encode:
            ...
            #     min_radius=10
            # )
            from SpatialRelationEncoder import GridCellSpatialRelationLocationEncoder
                ...
                try:
                    ...
                    for shipno, ship_data in df.groupby('shipno'):
                        ...
                        if 'lat' in self.features and 'lon' in self.features:
                            ...
                            if self.geo_encode:
                                # 地理编码
                                geo_coords_3d = geo_coords.reshape(-1, 1, 2)
                                geo_features = self.geo_encoder.forword(geo_coords_3d)
                                geo_features = geo_features.squeeze(1)
                            else:
                                # 直接使用原始坐标
...
def ship_data_provider(args, flag):
    ...
    # 使用的特征列
    features = ['lat', 'lon', 'sog', 'cog','delta_time','sin_timeofday','cos_timeofday']  # 可以根据需要调整
