...
class Exp_Ship_Classification(Exp_Basic):
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        self.model.eval()
        with torch.no_grad():
            for i, (batch_x, batch_y,batch_filenames,geo_embedding) in enumerate(vali_loader):
                batch_x = batch_x.float().to(self.device)
                batch_y = batch_y.long().squeeze(-1).to(self.device)  # [batch_size]
                geo_embedding = geo_embedding.float().to(self.device)
    ...
    def train(self, setting):
        ...
        for epoch in range(start_epoch, self.args.train_epochs):
            ...
            for i, (batch_x, batch_y,batch_filenames,geo_embedding) in enumerate(train_loader):
                iter_count += 1
            ...
            # 学习率调整
            if torch.isnan(loss):  # 检测到NaN
                print(f"Warning: NaN loss at batch {epoch}, skipping...")
                continue  # 跳过该batch
            if getattr(self.args, 'lradj', 'TST') != 'TST':
...
#         self.model.eval()
#         with torch.no_grad():
#             for i, (batch_x, batch_y, batch_filenames) in enumerate(test_loader):
#                 batch_x = batch_x.float().to(self.device)
...
#         # 分类报告
#         if hasattr(test_data, 'class_names'):
...
#         }
#         np.save(os.path.join(results_path, 'test_results.npy'), test_results)
#         data = np.load(os.path.join(results_path, 'test_results.npy'), allow_pickle=True)
#         df = pd.DataFrame(data)
#         df.to_csv(os.path.join(results_path, 'test_results.csv'), index=False)  # index=False避免保存行索引
...
#         return test_results
    def test(self, setting, test=0):
        ...
        if test:
            ...
            if self.args.resume:
                # self.model.load_state_dict(
                #     torch.load(os.path.join('./trainOut/' + setting, f'checkpoint_{resume_epoch}.pth'), weights_only=False)
                # )
        ...
        self.model.eval()
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_filenames,geo_embedding) in enumerate(test_loader):
                batch_x = batch_x.float().to(self.device)
                batch_y = batch_y.long().squeeze(-1).to(self.device)
        ...
        if hasattr(train_data, 'get_label_names'):
            class_names = train_data.get_label_names().tolist()  # 转换为列表
        elif hasattr(train_data, 'label_encoder') and hasattr(test_data.label_encoder, 'classes_'):
            class_names = train_data.label_encoder.classes_.tolist()
        elif hasattr(train_data.dataset, 'get_label_names') if hasattr(test_data, 'dataset') else False:
            class_names = train_data.dataset.get_label_names().tolist()
        elif hasattr(train_data.dataset, 'label_encoder') if hasattr(test_data, 'dataset') else False:
            ...
            print("Available attributes in test_data:", [attr for attr in dir(test_data) if not attr.startswith('_')])
        ...
        results_df = pd.DataFrame({
            'filename': all_filenames,
            'predictions': pred_class_names,  # 使用类别名称而不是索引
            'true_labels': true_class_names,  # 使用类别名称而不是索引
            'pred_index': all_preds,  # 保留原始索引以备需要
            'true_index': all_labels,  # 保留原始索引以备需要
            **prob_columns  # 展开概率列
        })
        ...
        predictions_df = pd.DataFrame({
            ...
            'shiptype': pred_class_names,  # 使用类别名称而不是索引
        })
        ...
        class_metrics_df = pd.DataFrame({
            'class_name': class_names[:len(precision)],  # 确保长度匹配
            'precision': precision,
            'recall': recall,
            'f1_score': f1,
            'support': support
