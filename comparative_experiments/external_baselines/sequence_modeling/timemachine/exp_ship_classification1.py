...
class Exp_Ship_Classification(Exp_Basic):
    ...
    def _build_model(self):
        """构建分类模型"""
        # 设置模型配置
        self.args.n_features = 4  # lat, lon, sog, cog
    ...
    def test(self, setting, test=0):
        ...
        # 获取类别名称 - 根据您的数据加载器结构
        if hasattr(test_data, 'get_label_names'):
            class_names = test_data.get_label_names().tolist()  # 转换为列表
        elif hasattr(test_data, 'label_encoder') and hasattr(test_data.label_encoder, 'classes_'):
            class_names = test_data.label_encoder.classes_.tolist()
        elif hasattr(test_data.dataset, 'get_label_names') if hasattr(test_data, 'dataset') else False:
            class_names = test_data.dataset.get_label_names().tolist()
        elif hasattr(test_data.dataset, 'label_encoder') if hasattr(test_data, 'dataset') else False:
            class_names = test_data.dataset.label_encoder.classes_.tolist()
        else:
            # 如果找不到类别名称，使用数字标签
