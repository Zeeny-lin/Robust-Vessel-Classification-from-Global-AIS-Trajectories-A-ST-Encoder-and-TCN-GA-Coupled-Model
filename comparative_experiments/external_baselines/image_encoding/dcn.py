"""
Function: Trajectory image + DCN training + progress bar + post-voting merge (ablation - DCN)
Model   : DCN (Dilated Convolution Network)
...
RESULT_DIR = r"D:\日常项目\25.08论文\resultprocess\DCN"
...
class DCNBlock(nn.Module):
    """Dilated Convolution block with multi-dilation fusion."""

    def __init__(self, channels):
        super().__init__()
        self.b1 = ConvBNAct(channels, channels, k=3, d=1)
        self.b2 = ConvBNAct(channels, channels, k=3, d=2)
        self.b3 = ConvBNAct(channels, channels, k=3, d=3)
        self.fuse = ConvBNAct(channels * 3, channels, k=1, p=0)
    ...
    def forward(self, x):
        ...
        return F.relu(y + x, inplace=True)
...
def train_one_epoch(model, loader, criterion, optimizer, scaler, scheduler, epoch, total_epochs):
    ...
    for batch_idx, (batch_X, batch_Y, filenames) in enumerate(pbar):
        ...
        pbar.set_postfix(
            {"Loss": f"{running_loss / n:.4f}", "Acc": f"{running_acc / n:.4f}", "Batch": f"{batch_idx+1}/{len(loader)}"}
        )
...
def test_model(model, test_loader, model_name=""):
    ...
    results_df = pd.DataFrame(
        {"traj_no": all_filenames, "test_shiptype": [CLASS_NAMES[p] for p in all_preds], "real_shiptype": [CLASS_NAMES[l] for l in all_labels]}
    )
    return accuracy, precision, recall, f1_val, results_df


def integrate_results(test_results_list, weights):
    vote_counts, real_type_map = {}, {}
    for (f1_val, epoch, _), results_df in zip(weights, test_results_list):
        for _, row in results_df.iterrows():
            ...
            if traj_no not in vote_counts:
                vote_counts[traj_no] = {}
            vote_counts[traj_no][pred_type] = vote_counts[traj_no].get(pred_type, 0) + f1_val
            real_type_map[traj_no] = real_type

    integrated_results = []
    for traj_no, votes in vote_counts.items():
        final_pred = max(votes.items(), key=lambda x: x[1])[0]
        integrated_results.append({"traj_no": traj_no, "test_shiptype": final_pred, "real_shiptype": real_type_map.get(traj_no, "Unknown")})
...
def main():
    ...
    with open(LOG_FILE, "w", encoding="utf-8") as f:
        f.write("DCN Training Log (Image-based Classification)\n")
    ...
    model = DCNClassifier(num_classes=len(CLASS_NAMES), dropout_rate=DROPOUT_RATE).to(device)
    ...
    criterion = nn.CrossEntropyLoss(weight=class_weights_tensor if USE_CLASS_WEIGHTS else None, label_smoothing=LABEL_SMOOTHING)
    optimizer = optim.AdamW(model.parameters(), lr=LEARNING_RATE, betas=(0.9, 0.99), weight_decay=WEIGHT_DECAY)
    if USE_ONECYCLE_LR:
        ...
        scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=TOTAL_EPOCHS, eta_min=MIN_LEARNING_RATE)
    ...
    for epoch in range(total_epochs):
        tr_loss, tr_acc = train_one_epoch(model, train_loader, criterion, optimizer, scaler, scheduler, epoch, total_epochs)
        val_loss, val_acc, val_precision, val_recall, val_f1, _, _ = evaluate_model(model, val_loader, criterion, epoch, total_epochs, "Val")
    ...
    for i, (f1_val, epoch, path) in enumerate(ckpts):
        ...
        results_df.to_csv(os.path.join(RUN_DIR, f"test_results_epoch_{epoch}.csv"), index=False, encoding="utf-8")
        test_metrics.append(
            {
                "epoch": epoch,
                "val_f1": f1_val,
    ...
    final_integrated_df.to_csv(os.path.join(RUN_DIR, "integrated_result.csv"), index=False, encoding="utf-8")
    ...
    integrated_precision = precision_score(eval_df["real_shiptype"], eval_df["test_shiptype"], average="weighted", zero_division=0)
    integrated_recall = recall_score(eval_df["real_shiptype"], eval_df["test_shiptype"], average="macro", zero_division=0)
    integrated_f1 = f1_score(eval_df["real_shiptype"], eval_df["test_shiptype"], average="weighted", zero_division=0)
