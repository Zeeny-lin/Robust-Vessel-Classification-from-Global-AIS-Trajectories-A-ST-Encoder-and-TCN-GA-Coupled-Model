"""
功能   : 轨迹 + 时间特征 训练 + 进度条 + 后处理投票合并 (消融实验 - SVM)
         模型：SVM（支持向量机）
...
RESULT_DIR = r'D:\日常项目\25.8论文\result\SVM_Ablation'
...
def main():
    ...
    with open(LOG_FILE, 'w') as f:
        f.write("SVM Ablation Study Training Log\n")
    ...
    # 2. 初始化模型包装器
    model_wrapper = SVMClassifierWrapper(num_classes=len(CLASS_NAMES))
