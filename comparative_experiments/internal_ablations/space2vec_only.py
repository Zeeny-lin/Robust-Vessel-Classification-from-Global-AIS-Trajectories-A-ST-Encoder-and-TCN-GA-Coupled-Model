"""
消融实验: 仅Space2Vec编码 (无TCN, 无Transformer, 无注意力机制)
Space2Vec编码空间特征(lat, lon) -> 全局池化 -> 分类头
目的: 验证Space2Vec单独的贡献, 预期效果较差
"""
...
RESULT_DIR = r'D:\日常项目\25.8论文\resultprocess\Ablation_Space2VecOnly'
...
class Space2VecOnlyModel(nn.Module):
    """
    消融模型: 仅使用Space2Vec编码空间特征, 无任何序列建模和注意力。
    数据流: lat,lon -> Space2Vec(64维) -> 全局平均池化 -> FC分类头
    其余特征(sog, cog, delta_h, day_frac)完全不使用。
    """
    def __init__(self, num_classes, spatial_embed_dim=SPATIAL_EMBED_DIM, dropout=0.1):
        super().__init__()
        
        print("\n=== Ablation: Space2Vec Only (NO TCN, NO Attention) ===")
        ...
        print(f"  Space2Vec: (lat, lon) -> {spatial_embed_dim}维")
        ...
        self.classifier = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(spatial_embed_dim // 2, num_classes)
        )
        print(f"  Classifier: {spatial_embed_dim} -> {spatial_embed_dim // 2} -> {num_classes}")
        
        print(f"\n  数据流: lat,lon -> Space2Vec({spatial_embed_dim}) -> GlobalPool -> FC -> 分类")
        print(f"  注意: sog, cog, delta_h, day_frac 完全不使用!")
        print("="*50 + "\n")
        
    def forward(self, x, lengths=None, return_attention_weights=False):
        spatial_coords = x[:, :, 0:2]  # 只取 lat, lon
        
        encoded = self.space2vec(spatial_coords)  # (batch, seq_len, spatial_embed_dim)
        
        # 全局池化
        encoded = encoded.permute(0, 2, 1)  # (batch, embed_dim, seq_len)
        pooled = self.global_pool(encoded).squeeze(-1)  # (batch, embed_dim)
...
def ensemble_predict(models_with_weights, test_loader):
    ...
    for (f1_weight, _, model_path) in models_with_weights:
        model = Space2VecOnlyModel(num_classes=len(CLASS_NAMES)).to(device)
...
def main():
    with open(LOG_FILE, 'w') as f:
        f.write(f"Ablation Study: Space2Vec Only (NO TCN, NO Attention)\n")
    ...
    log("Loading Data for Ablation (Space2Vec Only)...")
    ...
    model = Space2VecOnlyModel(num_classes=len(CLASS_NAMES), dropout=0.1).to(device)
    ...
    print("\n" + "="*50)
    print("ABLATION: SPACE2VEC ONLY (NO TCN, NO ATTENTION):")
    ...
    import json
    config = {
        'model_type': 'Space2VecOnly (Ablation)',
        'description': 'Only Space2Vec encoding on lat/lon, no TCN, no attention. Direct to classifier.',
        'features_used': ['lat', 'lon'],
        'features_ignored': ['sog', 'cog', 'delta_h', 'day_frac'],
