"""
消融实验: 仅时间特征编码 (无TCN, 无Transformer, 无注意力机制, 无Space2Vec)
时间特征(sog, cog, delta_h, day_frac) -> 线性投影 -> 全局池化 -> 分类头
目的: 验证时间特征编码单独的贡献, 预期效果较差
"""
...
RESULT_DIR = r'D:\日常项目\25.8论文\resultprocess\Ablation_TemporalOnly'
...
class TemporalOnlyModel(nn.Module):
    """
    消融模型: 仅使用时间特征的线性投影, 无任何序列建模和注意力。
    数据流: sog,cog,delta_h,day_frac -> Linear(4->32) -> 全局平均池化 -> FC分类头
    空间特征(lat, lon)完全不使用。
    """
    def __init__(self, num_classes, temporal_dim=TEMPORAL_DIM, dropout=0.1):
        super().__init__()
        
        print("\n=== Ablation: Temporal Encoding Only (NO TCN, NO Attention, NO Space2Vec) ===")
        
        # 时间特征投影 (与原模型中的temporal_proj结构一致)
        self.temporal_proj = nn.Sequential(
            nn.Linear(4, temporal_dim),
            nn.ReLU(),
            nn.Dropout(dropout)
        )
        print(f"  Temporal Projection: (sog, cog, delta_h, day_frac) 4 -> {temporal_dim}")
        ...
        print(f"  Classifier: {temporal_dim} -> {temporal_dim // 2} -> {num_classes}")
        
        print(f"\n  数据流: sog,cog,delta_h,day_frac -> Linear({temporal_dim}) -> GlobalPool -> FC -> 分类")
        print(f"  注意: lat, lon 完全不使用!")
        print("="*50 + "\n")
        
    def forward(self, x, lengths=None, return_attention_weights=False):
        temporal_features = x[:, :, 2:6]  # 只取 sog, cog, delta_h, day_frac
        
        encoded = self.temporal_proj(temporal_features)  # (batch, seq_len, temporal_dim)
        
        # 全局池化
        encoded = encoded.permute(0, 2, 1)  # (batch, temporal_dim, seq_len)
        pooled = self.global_pool(encoded).squeeze(-1)  # (batch, temporal_dim)
        
        output = self.classifier(pooled)
        
        if return_attention_weights:
            seq_len = x.size(1)
            dummy_weights = torch.ones(x.size(0), seq_len, device=x.device) / seq_len
            return output, {'spatial': dummy_weights, 'temporal': dummy_weights, 'cross': dummy_weights}
...
def ensemble_predict(models_with_weights, test_loader):
    ...
    for (f1_weight, _, model_path) in models_with_weights:
        model = TemporalOnlyModel(num_classes=len(CLASS_NAMES)).to(device)
...
def main():
    with open(LOG_FILE, 'w') as f:
        f.write(f"Ablation Study: Temporal Encoding Only (NO TCN, NO Attention, NO Space2Vec)\n")
    ...
    log("Loading Data for Ablation (Temporal Encoding Only)...")
    ...
    model = TemporalOnlyModel(num_classes=len(CLASS_NAMES), dropout=0.1).to(device)
    ...
    log("\n=== Training: Temporal Encoding Only ===")
    ...
    print("\n" + "="*50)
    print("ABLATION: TEMPORAL ENCODING ONLY (NO TCN, NO ATTENTION, NO SPACE2VEC):")
    ...
    import json
    config = {
        'model_type': 'TemporalOnly (Ablation)',
        'description': 'Only temporal feature projection (Linear 4->32), no TCN, no attention, no Space2Vec. Direct to classifier.',
        'features_used': ['sog', 'cog', 'delta_h', 'day_frac'],
        'features_ignored': ['lat', 'lon'],
