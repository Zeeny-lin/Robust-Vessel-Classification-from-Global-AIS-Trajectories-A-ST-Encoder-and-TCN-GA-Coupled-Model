"""
消融实验: 时间编码 + 空间线性投影 + 三层注意力(GA) (无TCN, 无Transformer, 无Space2Vec)
lat,lon -> Linear(2->spatial_dim), sog,cog,delta_h,day_frac -> Linear(4->temporal_dim)
-> 三层注意力(Spatial+Temporal+Cross) -> 分类头
"""
...
DATA_ROOT = r'D:\日常项目\25.08论文\data\data\process_seg'
TRAIN_DIR = os.path.join(DATA_ROOT, 'train')
VAL_DIR = os.path.join(DATA_ROOT, 'val')
TEST_DIR = os.path.join(DATA_ROOT, 'test')

RESULT_DIR = r'D:\日常项目\25.08论文\resultprocess\Ablation_Temporal_GA'
...
class TemporalGAModel(nn.Module):
    """
    时间编码 + 三层注意力(GA), 无TCN, 无Space2Vec
    数据流: lat,lon -> Linear(2->spatial_dim), sog,cog,delta_h,day_frac -> Linear(4->temporal_dim)
          Spatial/Temporal/Cross Attention -> 分类头
    """
    def __init__(self, num_classes, spatial_dim=SPATIAL_DIM, temporal_dim=TEMPORAL_DIM, dropout=0.1):
        super().__init__()
        print("\n=== Ablation: Spatial Linear + Temporal Encoding + GA (NO TCN, NO Space2Vec) ===")
        
        self.spatial_proj = nn.Sequential(
            nn.Linear(2, spatial_dim), nn.ReLU(), nn.Dropout(dropout))
        print(f"  Spatial Proj: lat,lon (2) -> {spatial_dim}")
        
        self.temporal_proj = nn.Sequential(
            nn.Linear(4, temporal_dim), nn.ReLU(), nn.Dropout(dropout))
        print(f"  Temporal Proj: sog,cog,delta_h,day_frac (4) -> {temporal_dim}")
        
        self.spatial_attention = SpatialAttention(spatial_dim, dropout=dropout)
        self.temporal_attention = TemporalAttention(temporal_dim, dropout=dropout)
        cross_hidden = 16
        self.cross_attention = CrossAttention(spatial_dim, temporal_dim, hidden_dim=cross_hidden, dropout=dropout)
        cross_out = cross_hidden * 2
        print(f"  GA: Spatial({spatial_dim}) + Temporal({temporal_dim}) + Cross -> {cross_out}")
        ...
        print("  Note: lat, lon use linear projection (no Space2Vec)")
        print("="*50 + "\n")

    def forward(self, x, lengths=None, return_attention_weights=False):
        spatial_enc = self.spatial_proj(x[:, :, 0:2])
        temporal_enc = self.temporal_proj(x[:, :, 2:6])
        
        if return_attention_weights:
            sa, sw = self.spatial_attention(spatial_enc, True)
            ta, tw = self.temporal_attention(temporal_enc, True)
            ca, cw = self.cross_attention(sa, ta, True)
        else:
            sa = self.spatial_attention(spatial_enc, False)
            ta = self.temporal_attention(temporal_enc, False)
...
def ensemble_predict(models_with_weights, test_loader):
    ...
    for (w, _, path) in models_with_weights:
        m = TemporalGAModel(num_classes=len(CLASS_NAMES)).to(device)
...
def main():
    with open(LOG_FILE,'w') as f: f.write(f"Ablation: Temporal + GA\nStart: {datetime.now()}\n")
    ...
    model = TemporalGAModel(num_classes=len(CLASS_NAMES), dropout=0.1).to(device)
    ...
    print(f"\n{'='*50}\nABLATION: Temporal Encoding + GA (NO TCN, NO Space2Vec):\nAccuracy: {acc:.4f}\nPrecision: {prec:.4f}\nRecall: {rec:.4f}\nF1: {f1:.4f}\n{'='*50}")
    ...
    import json
    json.dump({'model_type':'Temporal+GA','features_used':['sog','cog','delta_h','day_frac'],'features_ignored':['lat','lon'],
